...
def get_all_products(db: Session = Depends(get_db)):
    ...
    **Para el LLM**: Esta función lista todos los productos de la tienda
    """
...
def get_product_by_id(product_id: str, db: Session = Depends(get_db)):
    ...
    **Para el LLM**: Esta función obtiene detalles de un producto específico
...
def get_product_stock(product_id: str, db: Session = Depends(get_db)):
    ...
    **TOOL CALLING**: Esta es la función que el LLM llamará cuando necesite verificar stock
...
def get_product_pricing(product_id: str, db: Session = Depends(get_db)):
    ...
    **TOOL CALLING**: Esta función puede ser llamada por el LLM para consultar precios
...
def search_products(
    ...
    """
    Busca productos por nombre o descripción (optimizado para Tool Calling)
    
    **TOOL CALLING**: Esta función permite al LLM buscar productos específicos
...
def get_catalog_summary(db: Session = Depends(get_db)):
    """
    Obtiene un resumen ligero del catálogo para RAG
    
    **RAG**: Este endpoint proporciona contexto sin sobrecargar el prompt del LLM
    Retorna solo: ID, nombre y precio (sin descripciones largas)
    
    Returns:
        Lista simplificada de productos para inyectar en el System Prompt
    """
    products = db.query(
        Product.id,
        Product.name,
